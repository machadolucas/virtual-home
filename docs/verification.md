...
| Passkeys: RP ID from `VH_BASE_URL`, signed assertion signs an active member in, inactive member refused with no session, unknown credential refused, registration refused for a session older than 10 min (password- and passkey-minted sessions both fresh), CLI list/remove, reset keeps passkeys, owner-only audited removal, `lastUsedAt` stamped on a minted session and not on a refused one, each options call leaves one 5-minute challenge row, expiry pruning keeps live challenges (a sign-in completes across a prune), full registration ceremony with the all-zero AAGUID named from the user agent ("Safari on iPhone") | `tests/unit/auth/passkey.test.ts` (software authenticator against the real endpoints); expired-row deletes in `tests/unit/worker/jobs.test.ts` | **15 passed** (2026-09-25) |
| Security page session list: every active session past Better Auth's 100-row `listSessions` default (152 rows), current first then newest, expired / impersonated / other users' sessions excluded | `tests/unit/auth/session-list.test.ts` | **2 passed** (2026-09-25) |
| Browser, passkeys: Permissions-Policy, register in Security, conditional-UI sign-in after sign-out, button sign-in from a fresh browser, last-used shown, rename, delete, deleted passkey refused | `tests/e2e/passkey.spec.ts` (Chromium projects; CDP virtual authenticator) | **2 passed** — desktop + phone (2026-09-24) |
