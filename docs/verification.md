...
| Passkeys: RP ID from `VH_BASE_URL`, signed assertion signs an active member in, inactive member refused with no session, unknown credential refused, registration refused for a session older than 10 min (password- and passkey-minted sessions both fresh), CLI list/remove, reset keeps passkeys, owner-only audited removal | `tests/unit/auth/passkey.test.ts` (software authenticator against the real endpoints) | **12 passed** (2026-09-25) |
| Browser, passkeys: Permissions-Policy, register in Security, conditional-UI sign-in after sign-out, button sign-in from a fresh browser, rename, delete, deleted passkey refused | `tests/e2e/passkey.spec.ts` (Chromium projects; CDP virtual authenticator) | **2 passed** — desktop + phone (2026-09-24) |
