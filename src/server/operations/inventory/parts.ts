...
export const createPart = action(createPartInput, (input, session) => {
  const { db } = getDb();
...
export const updatePart = action(updatePartInput, (input, session) => {
  const { db } = getDb();
...
export const setKitComponents = action(setKitComponentsInput, (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
...
export const upsertSupplier = action(upsertSupplierInput, (input, session) => {
  const { db } = getDb();
...
export const removeSupplier = action(removeSupplierInput, (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
...
export const upsertLot = action(upsertLotInput, (input, session) => {
  const { db } = getDb();
...
export const setPartArchived = action(archivePartInput, (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
