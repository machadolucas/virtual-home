...
import {
  ...
  setConsumablesInput,
  updateAssetInput,
} from "@/server/actions/assets/schemas";
...
export const createEquipment = action(createAssetInput, (input, session) => {
  const { db } = getDb();
...
export const updateEquipment = action(updateAssetInput, (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
      ...
      if(input.isVirtual&&input.locationId!=null)throw new ValidationError("virtual_asset_has_location","A software unit has no room.");
...
export const setConsumables = action(setConsumablesInput, (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
 ...
 */
export const retireEquipment = action(retireAssetInput, (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
...
export const replaceEquipment = action(replaceAssetInput, (input, session) => {
  const { db } = getDb();
