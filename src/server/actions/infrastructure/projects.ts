...
export const createProject = action(createProjectInput, async (input, session) => {
  const { db } = getDb();
...
export const updateProject = action(updateProjectInput, async (input, session) => {
  const { db } = getDb();
  const startedOn = input.startedOn ?? null;
  const endedOn = input.endedOn ?? null;
  mapDomainErrors(() => assertDates(startedOn, endedOn));
 ...
 */
export const deleteProject = action(deleteProjectInput, async (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
...
export const addProjectLink = action(addProjectLinkInput, async (input, session) => {
  const { db } = getDb();
  const id = mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
...
export const removeProjectLink = action(removeProjectLinkInput, async (input, session) => {
  const { db } = getDb();
 ...
 */
export const addProjectAttachment = action(addProjectAttachmentInput, async (input, session) => {
  const { db } = getDb();
  mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
