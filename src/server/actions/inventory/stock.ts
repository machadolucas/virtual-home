...
export const addPurchase = action(addPurchaseInput, async (input, session) => {
  const { db } = getDb();
 ...
 */
export const stockTake = action(stockTakeInput, async (input, session) => {
  const { db } = getDb();
  const result = mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
...
export const explodeKit = action(explodeKitInput, async (input, session) => {
  const { db } = getDb();
  const result = mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
...
export const undoExplode = action(undoExplodeInput, async (input, session) => {
  const { db } = getDb();
  const result = mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
 ...
 */
export const correctTransaction = action(correctTransactionInput, async (input, session) => {
  const { db } = getDb();
...
export const setEstimate = action(setEstimateInput, async (input, session) => {
  const { db } = getDb();
  const result = mapDomainErrors(() =>
    writeTx(db, (tx) => {
      const ctx = userContext(session, tx);
