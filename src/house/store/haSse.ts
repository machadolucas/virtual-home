  ...
  const parse = (raw: string): void => {
    ...
    for (const item of frame.items ?? []) {
      ...
      events.push({
        ...
        targetTempLow: numberOrNull(attributes.target_temp_low),
        targetTempHigh: numberOrNull(attributes.target_temp_high),
        temperatureUnit: stringOrNull(attributes.temperature_unit ?? attributes.unit_of_measurement),
        hvacAction: stringOrNull(attributes.hvac_action),
