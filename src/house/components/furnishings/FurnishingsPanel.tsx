...
export function FurnishingsPanel({ showItems = true }: { showItems?: boolean }) {
  ...
  return <details open={expanded} onToggle={(event) => setExpanded(event.currentTarget.open)} className="rounded-md border border-line bg-surface-2">
    ...
    <div className="flex flex-col gap-2 border-t border-line p-2">
      ...
      {!showItems && items.length > 0 ? <p className="text-xs text-ink-3">Select furniture in the property tree to edit or delete it.</p> : null}
      {showItems ? items.map((item) => <div key={item.id} className="flex min-w-0 items-center gap-1">
