...
export function Lighting() {
  ...
  const luxEntity = useStore(haStore, (s) =>
    settings.outdoorLuxEntityId ? s.entities[settings.outdoorLuxEntityId] : undefined,
  );
  const weatherEntity = useStore(haStore, (s) =>
    settings.weatherEntityId ? s.entities[settings.weatherEntityId] : undefined,
  );
  const haConnection = useStore(haStore, (s) => s.connection);
  ...
  const baseAppearance = daylightAppearance(solar.elevationDeg);
  const environmentEnabled = settings.mode === "live" && !studio;
  const luxLive = environmentEnabled && classifyState(luxEntity, haConnection, now) === "live";
  const weatherLive = environmentEnabled && classifyState(weatherEntity, haConnection, now) === "live";
  const lux = luxLive && luxEntity ? outdoorLuxValue(luxEntity.state, luxEntity.unit) : null;
  const appearance = adaptDaylightToEnvironment(baseAppearance, {
    lux: Number.isFinite(lux) && lux! >= 0 ? lux : null,
    weather: weatherLive ? weatherEntity?.state : null,
  });
  ...
  const calibratedIntensity = settings.intensity * GLOBAL_ILLUMINATION_BASELINE;
  const intensity = (studio ? 1.1 : appearance.sunIntensity + appearance.nightFillIntensity) * calibratedIntensity;
  ...
  useEffect(() => {
    ...
    invalidate();
  }, [sun, manifest, dx, dy, dz, performance, settings.softShadows, gl, invalidate]);

  // Photometric HA updates do not change the shadow camera or occluders, so reuse its map.
  useEffect(() => {
    sun.intensity = intensity;
    sun.color.set(color);
    invalidate();
  }, [sun, intensity, color, invalidate]);

  useEffect(() => () => sun.dispose(), [sun]);
  return (
    <>
      <hemisphereLight args={[studio ? "#ffffff" : appearance.skyColor, studio ? "#88806a" : appearance.groundColor, (studio ? 0.8 : appearance.ambientIntensity) * calibratedIntensity]} />
      <primitive object={sun} />
      <primitive object={sun.target} />
      {studio && <directionalLight position={[-10, 8, 12]} intensity={0.3 * calibratedIntensity} />}
    </>
  );
}
