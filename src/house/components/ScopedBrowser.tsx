...
export function ScopedBrowser({ searchRef, onInspect }: { searchRef?: RefObject<HTMLInputElement | null>; onInspect?:()=>void }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-3">
    ...
    <div hidden={activeCreation||!!add} className="min-h-0">
      ...
      {furniture.error ? <div role="alert" className="mb-2 flex items-center gap-2 rounded-md border border-line bg-surface-2 p-2 text-xs text-overdue"><span className="min-w-0 flex-1">{furniture.error}. Furniture is missing from this list and the model until it loads.</span><Button size="sm" variant="secondary" onClick={furniture.retry}>Retry</Button></div> : null}
