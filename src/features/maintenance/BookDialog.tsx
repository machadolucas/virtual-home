...
import { Button, Checkbox, Dialog, Field, Input, Textarea } from "@/ui";
import { ProviderPicker } from "@/features/providers/ProviderPicker";
import { bookProfessional } from "@/server/actions/maintenance/bookings";
...
export function BookDialog({
  ...
  return (
    <Dialog
      open={open}
      onOpenChange={(next) => { if (!booking.pending) onOpenChange(next); }}
      title="Book a professional"
      description="Record who is coming and when. You will log the completed work separately."
      footer={
        <>
          <Button variant="ghost" disabled={booking.pending} onClick={() => onOpenChange(false)}>
            Cancel
          </Button>
          <Button
            variant="primary"
            loading={booking.pending}
            disabled={!canSubmit}
    ...
    >
      <div className="flex flex-col gap-4">
        <Field label="Provider" required>
          {({ id, describedBy }) => (
            <ProviderPicker id={id} describedBy={describedBy} value={providerId} onValueChange={setProviderId} options={providers.map((row) => ({ value: row.id, label: row.name, hint: [row.trade, row.phone].filter(Boolean).join(" · ") || undefined }))} />
          )}
        </Field>
