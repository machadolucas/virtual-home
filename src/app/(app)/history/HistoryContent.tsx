...
export default async function HistoryPage(props: {
  ...
  const members = loadMembers(db, { includeInactive: true });

  return (
    <PageScroll>
      <PageHeader
        eyebrow="Household"
        title={filters.completionId ? rows[0]?.title ?? "Completion unavailable" : "History"}
        ...
        actions={
          <>
            <a
              download href="/api/exports/maintenance?format=json"
            ...
            <a
              download href="/api/exports/maintenance?format=csv&dataset=completions"
      ...
      {filters.completionId && <p className="text-sm text-ink-2">Showing one recorded completion. <Link href="/history" className="text-accent-text underline">View all history</Link>{rows[0]?.occurrenceId && <> · <Link href={`/tasks/${rows[0].occurrenceId}#completion-${filters.completionId}`} className="text-accent-text underline">Open maintenance task</Link></>}</p>}
      {!filters.completionId && <HistoryFilters
        targets={targets}
        from={filters.from}
        to={filters.to}
      ...
      {rows.length === 0 ? (
        ...
        >
          <ul className="divide-y divide-line md:hidden">{rows.map((row) => <li key={`${row.type}-${row.id}`} className="p-4"><div className="flex items-center justify-between gap-2"><Badge tone={TYPE_BADGE[row.type].tone} size="sm">{TYPE_BADGE[row.type].label}</Badge><time className="text-xs text-ink-3">{row.date ? formatDate(row.date) : "No date agreed"}</time></div><p className="mt-2 font-medium">{row.occurrenceId ? <Link href={(row.type === "completions" || row.type === "voided" ? `/history/completions/${row.id}` : `/tasks/${row.occurrenceId}`) as Route} className="text-accent-text">{row.title}</Link> : row.title}</p><p className="mt-1 text-sm text-ink-2">{[row.actorProviderName ?? members.find((member) => member.id === row.actorUserId)?.name, row.target?.name].filter(Boolean).join(" · ")}</p><details className="mt-2 text-sm"><summary className="cursor-pointer py-2 text-ink-2">Entry details</summary><div className="space-y-2 pb-2">{row.bookingStatus && <p>Booking: {row.bookingStatus.replaceAll("_", " ")}</p>}{row.voidReason && <p className="text-overdue">Voided: {row.voidReason}</p>}{row.reason && <p>{row.reason}</p>}{row.notes && <p className="whitespace-pre-wrap">{row.notes}</p>}{row.effortMinutes !== null && <p>{formatMinutes(row.effortMinutes)}</p>}{row.materials.map((line, i) => <p key={i}>{line.partName} · {formatQty(line.actualQtyMilli, line.unit)}{line.shortfallMilli > 0 ? ` · ${formatQty(line.shortfallMilli, line.unit)} short` : ""}</p>)}<div className="flex flex-wrap gap-2">{row.photoIds.map((id, i) => <DocumentLink key={id} document={{id}}>Photo {i + 1}</DocumentLink>)}</div>{row.type === "completions" || row.type === "voided" ? <AddToProject entityKind="completion" entityId={row.id} /> : row.occurrenceId && <AddToProject entityKind="occurrence" entityId={row.occurrenceId} />}</div></details></li>)}</ul>
                      ...
                      <td className="px-4 py-2.5 align-top">
                        <div className="flex flex-col gap-1">
                          ...
                          {row.occurrenceId === null ? (
                            ...
                            <Link
                              href={(row.type === "completions" || row.type === "voided" ? `/history/completions/${row.id}` : `/tasks/${row.occurrenceId}`) as Route}
