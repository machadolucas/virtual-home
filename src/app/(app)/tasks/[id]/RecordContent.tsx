...
export default async function TaskPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (task === null) return <p role="status">This record is unavailable. It may have been removed. Close this view to continue.</p>;

  const members = loadMembers(db);
  const historicalMembers = loadMembers(db, { includeInactive: true });
  ...
  return (
    <PageScroll>
            ...
            <div>
              ...
              <dd className="text-ink">
                {task.liveCompletion.performedByProviderName ??
                  historicalMembers.find((m) => m.id === task.liveCompletion?.performedByUserId)?.name ??
                  "Not recorded"}
                {task.liveCompletion.recordedBy !== null &&
                task.liveCompletion.recordedBy !== task.liveCompletion.performedByUserId
                  ? ` · logged by ${historicalMembers.find((m) => m.id === task.liveCompletion?.recordedBy)?.name ?? "another member"}`
                  : ""}
              </dd>
            </div>
      ...
      <Timeline events={task.events} members={historicalMembers} tz={tz} />
      <PlanHistory entries={task.history} members={historicalMembers} tz={tz} />
