...
export default async function DocumentPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{gallery?:string}> }) {
  ...
  if (!file && !service) return <p role="status">This record is unavailable. It may have been removed. Close this view to continue.</p>;
  const query=await searchParams;
  const gallery=(query?.gallery??"").split(",").filter(Boolean).slice(0,100);
  const galleryIndex=gallery.indexOf(id);
  ...
  return <PageScroll><Link href="/documents" className="text-sm text-accent-text">← Documents</Link><PageHeader title={service?.documentNo || file?.caption || file?.originalFilename || service?.kind || "Document"} />
    {galleryIndex>=0 && gallery.length>1 && <nav aria-label="Document gallery" className="flex gap-3">{galleryIndex>0 && <Link scroll={false} href={`/documents/${gallery[galleryIndex-1]}?gallery=${encodeURIComponent(gallery.join(","))}`}>Previous file</Link>}<span>{galleryIndex+1} / {gallery.length}</span>{galleryIndex<gallery.length-1 && <Link scroll={false} href={`/documents/${gallery[galleryIndex+1]}?gallery=${encodeURIComponent(gallery.join(","))}`}>Next file</Link>}</nav>}
